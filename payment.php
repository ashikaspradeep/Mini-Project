...
<body>
    ...
    <form action="payment_action.php" method="post">
      <input type="text" name="name" placeholder="Enter Name Here" required>
            ...
            $id=$_GET['id'];
            $sql=mysqli_query($connection,"select * from booking_det where id='$id'");
            while($a=mysqli_fetch_array($sql))
            {
                ?>
                <input type="hidden" name="bid" value="<?php echo $a['id'];?>">
      <input type="text" name="amount" value="<?php echo $a['tamt'];?>" readonly>
      <?php
            }
            ?>
      <input type="text" name="cardnumber" placeholder="Enter cardnumber" maxlength="16" minlength="16" required>
